...
STATE  HybridMapping::getppn(const Addr lpn, Mask & mask, Addr & ppn){
	...
	if (physical_blocks[data_pbn].get_page_state(page_offset) == Block::PAGE_VALID){
		...
		physical_blocks[data_pbn].get_page_mask(page_offset, mask); 
		return SUCCESS; 
	}
	...
	if (physical_blocks[log_pbn].get_page_state(log_page_offset) == Block::PAGE_VALID){
		ppn = log_ppn; 
		physical_blocks[log_pbn].get_page_mask(log_page_offset, mask); 
		return SUCCESS; 
	}
...
STATE  HybridMapping::allocate_new_page(const Addr lpn, const Mask & mask, Addr & ppn){
    	if (lpn > param->logical_page_number){
        	my_assert("allocate_page: address out of bound");
        	return ERROR;
    	}
    
	STATE s = insert_into_data_block(lpn, mask, ppn); 

	if (s != SUCCESS){
		return insert_into_log_block(lpn, mask, ppn);
	}
	
	return s; 
}
STATE  HybridMapping::insert_into_data_block(const Addr lpn, const Mask & mask, Addr & ppn){
	...
	if (physical_blocks[data_block].write_page(lpn, mask, offset) == SUCCESS) {
		ppn = data_block * param->page_per_block + lpn % param->page_per_block;
		return SUCCESS;
	}
	return FAIL; 
}
STATE  HybridMapping::insert_into_log_block(const Addr lpn, const Mask & mask, Addr & ppn){
    ...
    if (i == K_Log){
        ...
        if (log_block_count < K_Log){
			...
			if (get_free_block(new_log_block) != SUCCESS || log_block_MT->addLogBlock(group_number, new_log_block) != SUCCESS){
				...
				my_assert("Fail to insert into log_block \n");
				return ERROR;
			}
    ...
    if((selected_log_block == -1) || selected_log_block > (param->physical_page_number / param->page_per_block)) {
    	printf( "FTL Error in insert_into_log_block \n");
	return ERROR; 
    }
    if (physical_blocks[selected_log_block].write_page(lpn, mask, page_offset) != SUCCESS) {    
    	printf( "FTL Error couldn't write \n");
        return ERROR;
    }
	...
	if (log_page_MT->add(group_number, lpn, ppn) != SUCCESS){
		my_assert("Fail to add new entry to LPMT ");
		return ERROR; 
	}
...
STATE HybridMapping::do_direct_erase(const Addr target_block, const Addr target_group, const Addr * target_lpns) {
	...
	Mask erase_mask(param->superpage_degree, true); 
	ftl->PAM_SendRequest(ftl->seq_num++, ftl->current_time, target_block * param->page_per_block, OP_ERASE, erase_mask, true); 
	direct_erase_count++; 
	return SUCCESS; 
} 
...
STATE HybridMapping::do_switch_merge(const Addr target_block, const Addr target_group, const Addr * target_lpns) {
	...
	Mask erase_mask(param->superpage_degree, true); 
	ftl->PAM_SendRequest(ftl->seq_num++, ftl->current_time, current_pbn * param->page_per_block, OP_ERASE, erase_mask, true); 
...
STATE HybridMapping::do_reorder_merge(const Addr target_block, const Addr target_group, const Addr * target_lpns) {
	...
	for (int i = 0; i < param->page_per_block; i++) {
		...
		Addr read_ppn;
		Mask move_mask(param->superpage_degree, true); 
		if (getppn(new_lpn, move_mask, read_ppn) == SUCCESS){
        	ftl->PAM_SendRequest(-1, ftl->current_time, read_ppn, OP_READ, move_mask, false); 
			map_gc_move_read_count++; 
		}
		invalid_old_page(new_lpn); // invalidate and remove from lpmt if exists 
			
		if (physical_blocks[new_pbn].write_page(new_lpn, move_mask, i) != SUCCESS){
			my_assert("Fail to write block! ");
			return FAIL;
		}
		Addr write_ppn = new_pbn * param->page_per_block + i;
		ftl->PAM_SendRequest(-1, ftl->current_time, write_ppn, OP_WRITE, move_mask, false);  
...
STATE HybridMapping::do_partial_merge(const Addr target_block, const Addr target_group, const Addr * target_lpns) {
    ...
    for (int i = physical_blocks[target_block].page_sequence_number; i < param->page_per_block; i++) {
		...
		int temp_i = i; 
		if (physical_blocks[target_block].write_page(new_lpn, move_mask, temp_i) == SUCCESS){
...
STATE HybridMapping::do_full_merge(const Addr target_block, const Addr target_group, const Addr * target_lpns) {
    for (int i = 0; i < param->page_per_block; i++){
		...
		for (int j = 0; j < param->page_per_block; j++) {
			...
			Addr copy_ppn;
			move_mask.set(true); 
			if (getppn(copy_lpn, move_mask, copy_ppn) == SUCCESS){
        		ftl->PAM_SendRequest(-1, ftl->current_time, copy_ppn, OP_READ, move_mask, false);
				map_gc_move_read_count++; 
			}
            
			int temp_j = j; 
			if (physical_blocks[free_block].write_page(copy_lpn, move_mask, temp_j) != SUCCESS) {
			...
			ftl->PAM_SendRequest(-1, ftl->current_time, new_ppn, OP_WRITE, move_mask, false); 
			map_gc_move_write_count++; 
		}
		...
		erase_block (current_pbn); 
		Mask erase_mask(param->superpage_degree, true); 
		ftl->PAM_SendRequest(-1, ftl->current_time, current_pbn * param->page_per_block, OP_ERASE, erase_mask, false);  
	...
	if (log_block_MT->removeLogBlock(target_group, target_block) != SUCCESS) {
   	...
   	erase_block(target_block);
	Mask erase_mask(param->superpage_degree, true);  	  
	ftl->PAM_SendRequest(ftl->seq_num++, ftl->current_time, target_group * param->page_per_block, OP_ERASE, erase_mask, true); 
	full_merge_count++;
...
// Random 
	...
	while (block_list[random] == -1){
		random = rand() % count; 
	}

	victim = block_list[random]; 
...
// Random ++ 
	/*
	int random = rand() % count;
	while (block_list[random] == -1 || physical_blocks[block_list[random]].valid_page_count() == physical_blocks[random].page_number){
		random = rand() % count;
	}
...
void HybridMapping::garbage_collect(){
...
void HybridMapping::PrintStats(){
	...
	printf( "FTL Map uses Hybrid Mapping parameters N: %d , K: %d \n", N_Data, K_Log);
	printf( "FTL Map tables size: \n");
	printf( "FTL Map Data Block Mapping Table: %lld KB\n", param->logical_page_number / param->page_per_block / (128)); // assume 8B for each entry
	printf( "FTL Map Log Block Mapping Table (Maximum): %d KB\n", N_Data * K_Log * 8 / KBYTE);
	printf( "FTL Map Log Page Mapping Table (Maximum): %d MB\n", N_Data * K_Log * param->page_per_block * 8 / MBYTE);
	
	printf( "FTL Map Hybrid direct erase count: %d \n", direct_erase_count);
	printf( "FTL Map Hybrid partial merge count: %d \n", partial_merge_count);
	printf( "FTL Map Hybrid reorder merge count: %d \n", reorder_merge_count);
	printf( "FTL Map Hybrid switch merge count: %d \n", switch_merge_count);
	printf( "FTL Map Hybrid full merge count: %d \n", full_merge_count);
	printf( "FTL Map Hybrid global merge count: %d \n", global_gc_count);
