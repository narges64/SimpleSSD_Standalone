...
MappingTable::MappingTable(FTL * f){
    	ftl = f; 
    	param = ftl->getParameter(); 
    ...
    for (Addr i = 0; i < param->physical_block_number; i++) {
    	physical_blocks[i].initialize(param->page_per_block, i, param->superpage_degree); 	
		add_free_block(i);
   	}
...
STATE MappingTable::read(const Addr lpn, Addr & ppn, Mask & mask){
	return getppn(lpn, mask, ppn);
}

STATE MappingTable::write(const Addr lpn, Addr & ppn, const Mask & mask){
    if (allocate_new_page(lpn, mask, ppn) != SUCCESS) {
		STATE merge_state = merge(lpn); 
		if (merge_state == SUCCESS) {
			return allocate_new_page(lpn, mask, ppn);
...
void MappingTable::PrintStats(){
	...
	printf( "FTL Map parameters: \n");
	printf( "FTL Map physical block count: %lld \n", tot_phy_blk_count);
	printf( "FTL Map logacal block count: %lld \n", tot_log_blk_count);
	printf( "FTL Map over-provisioning: %.2f \n", param->over_provide);
	printf( "FTL Map GC Strategy: GREEDY \n");
	...
	printf( "FTL Map Meta data Information for each block: %dB (lpn) + %dB (Valid bits, Bad bit, erase counter, write pointer)\n", param->page_per_block * 8, byte_per_block);
	
	printf( "FTL Map Total gc count %d \n", map_total_gc_count);
	printf( "FTL Map Total block erase count %d \n", map_block_erase_count);
	printf( "FTL Map GC move count (read, write)  %d  %d \n", map_gc_move_read_count, map_gc_move_write_count);
	printf( "FTL Map Bad block count %d \n", map_bad_block_count); // bad block count
	if (map_gc_lat_min  == DBL_MAX) 
		printf( "FTL Map GC latency (min , max , avg) : ( NA , NA , NA) us \n");
	else 
		printf( "FTL Map GC latency (min , max , avg) : ( %.2f , %.2f , %.2f) us \n", map_gc_lat_min, map_gc_lat_max, map_gc_lat_avg);
	printf( "FTL Map free/used block count: %lld/%lld\n", map_free_block_count, map_used_block_count); 
}
